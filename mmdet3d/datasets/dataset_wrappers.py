    ...
    def show(self, results, out_dir):
        ...
        for i, result in enumerate(results):
            ...
            for j in range(len(info['img_info'])):
                ...
                out_cur_path = os.path.split(info['img_info'][j]['filename'])[-2].split('/')[-1]
                out_path = os.path.join(out_dir, out_cur_path)
                if not os.path.exists(out_path):
                    os.makedirs(out_path)
                out_file_name = os.path.split(info['img_info'][j]['filename'])[-1][:-4]
                skimage.io.imsave(os.path.join(out_path, f'{out_file_name}.png'), img)